-- =============================================================================
-- Phase 2: Multi-Tenant RLS Rewrite
-- Drops ALL existing RLS policies and recreates with tenant_id scoping.
-- Every policy includes is_master_admin() bypass.
-- =============================================================================
...
-- tenants (new table, no existing policies)
...
-- TENANTS
-- -------------------------------------------------------------------
CREATE POLICY "master_admin_all_tenants" ON public.tenants
  FOR ALL USING (is_master_admin())
  WITH CHECK (is_master_admin());

CREATE POLICY "tenant_admin_read_own" ON public.tenants
  FOR SELECT USING (id = get_my_tenant_id());

CREATE POLICY "tenant_admin_update_own" ON public.tenants
  FOR UPDATE USING (
    id = get_my_tenant_id()
    AND EXISTS (
      SELECT 1 FROM user_roles
      WHERE user_id = auth.uid() AND role = 'admin' AND tenant_id = tenants.id
    )
  );
...
CREATE POLICY "user_roles_self_read" ON public.user_roles
  FOR SELECT USING (
    is_master_admin() OR user_id = auth.uid()
  );
...
CREATE POLICY "user_roles_admin_insert" ON public.user_roles
  FOR INSERT WITH CHECK (
    ...
    OR EXISTS (
      SELECT 1 FROM user_roles ur
      WHERE ur.user_id = auth.uid() AND ur.role = 'admin' AND ur.tenant_id = user_roles.tenant_id
    )
  );
...
CREATE POLICY "staff_self_update" ON public.staff_profiles
...
CREATE POLICY "staff_admin_read" ON public.staff_profiles
  FOR SELECT USING (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND has_any_role(ARRAY['admin'::app_role]))
  );
...
CREATE POLICY "staff_hr_read" ON public.staff_profiles
  FOR SELECT USING (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND has_any_role(ARRAY['hr'::app_role]))
  );

CREATE POLICY "staff_hr_insert" ON public.staff_profiles
  FOR INSERT WITH CHECK (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND has_any_role(ARRAY['hr'::app_role]))
  );

CREATE POLICY "staff_hr_update" ON public.staff_profiles
  FOR UPDATE USING (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND has_any_role(ARRAY['hr'::app_role]))
  ) WITH CHECK (
...
CREATE POLICY "staff_driver_read_for_messaging" ON public.staff_profiles
  FOR SELECT USING (
    is_master_admin()
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = staff_profiles.user_id
        AND ur.role IN ('admin', 'hr', 'finance')
      )
    )
  );
...
CREATE POLICY "dp_admin_insert" ON public.driver_profiles
  FOR INSERT WITH CHECK (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND has_any_role(ARRAY['admin'::app_role, 'hr'::app_role]))
  );
...
CREATE POLICY "dp_finance_read" ON public.driver_profiles
  FOR SELECT USING (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND has_any_role(ARRAY['finance'::app_role]))
  );
...
CREATE POLICY "routes_driver_read" ON public.routes
  FOR SELECT USING (
    ...
    OR (
      tenant_id = get_my_tenant_id()
      AND driver_id IN (SELECT id FROM driver_profiles WHERE user_id = auth.uid())
    )
  );
...
CREATE POLICY "routes_admin_all" ON public.routes
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role = 'admin' AND ur.tenant_id = routes.tenant_id
      )
    )
  );
...
CREATE POLICY "stops_admin_all" ON public.stops
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role = 'admin' AND ur.tenant_id = stops.tenant_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        ...
        AND ur.tenant_id = invoices.tenant_id
      )
    )
  );
...
CREATE POLICY "weekly_pay_admin_finance" ON public."WEEKLY_PAY"
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = "WEEKLY_PAY".tenant_id
      )
    )
  );
  ...
  FOR SELECT USING (
    ...
    OR (
      ...
      AND EXISTS (
        ...
        WHERE d.user_id = auth.uid() AND d.operator_id = "WEEKLY_PAY".operator_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = "DAILY_PAY_QTY".tenant_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = "DAILY_PAY_SUMMARY".tenant_id
      )
    )
  );
...
CREATE POLICY "daily_pay_summary_driver_read" ON public."DAILY_PAY_SUMMARY"
  FOR SELECT USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM driver_profiles d
        WHERE d.user_id = auth.uid() AND d.operator_id = "DAILY_PAY_SUMMARY".operator_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = "ADJUSTMENT_DETAIL".tenant_id
      )
    )
  );
...
CREATE POLICY "adj_detail_driver_read" ON public."ADJUSTMENT_DETAIL"
  FOR SELECT USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM driver_profiles d
        WHERE d.user_id = auth.uid() AND d.operator_id = "ADJUSTMENT_DETAIL".operator_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = "ADJUSTMENT_SUMMARY".tenant_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = payslips.tenant_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = earnings.tenant_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = deductions.tenant_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = pay_statements.tenant_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = supplier_rates.tenant_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = driver_rates.tenant_id
      )
    )
  );
...
CREATE POLICY "driver_rates_driver_read" ON public.driver_rates
  FOR SELECT USING (
    ...
    OR (
      tenant_id = get_my_tenant_id()
      AND driver_id = (SELECT id FROM driver_profiles WHERE user_id = auth.uid())
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = pay_rates.tenant_id
      )
    )
  );
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = rate_status.tenant_id
      )
    )
  );

CREATE POLICY "rate_status_authenticated_read" ON public.rate_status
  FOR SELECT USING (
...
CREATE POLICY "dispatchers_admin_all" ON public.dispatchers
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role = 'admin' AND ur.tenant_id = dispatchers.tenant_id
      )
    )
  );

CREATE POLICY "dispatchers_read_tenant" ON public.dispatchers
  FOR SELECT USING (
    is_master_admin() OR tenant_id = get_my_tenant_id()
  );
...
CREATE POLICY "messages_receiver_update" ON public.messages
  FOR UPDATE USING (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND receiver_id = auth.uid())
  );

-- -------------------------------------------------------------------
-- NOTIFICATIONS
-- -------------------------------------------------------------------
CREATE POLICY "notifications_participant_read" ON public.notifications
  FOR SELECT USING (
    is_master_admin()
    OR (
      tenant_id = get_my_tenant_id()
...
CREATE POLICY "notifications_insert" ON public.notifications
  FOR INSERT WITH CHECK (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND sender_id = auth.uid())
  );

CREATE POLICY "notifications_update" ON public.notifications
  FOR UPDATE USING (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND recipient_id = auth.uid())
  );
...
CREATE POLICY "notif_settings_self" ON public.notification_settings
  FOR ALL USING (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND user_id = auth.uid())
  ) WITH CHECK (
    is_master_admin()
    OR (tenant_id = get_my_tenant_id() AND user_id = auth.uid())
  );
...
CREATE POLICY "incidents_admin_read" ON public.incidents
  FOR SELECT USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role = 'admin' AND ur.tenant_id = incidents.tenant_id
      )
    )
  );
...
-- -------------------------------------------------------------------
-- COMPANY_DETAILS (legacy, keep basic access)
-- -------------------------------------------------------------------
  ...
  FOR ALL USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM user_roles ur
        WHERE ur.user_id = auth.uid() AND ur.role IN ('admin', 'finance')
        AND ur.tenant_id = internal_expenses.tenant_id
      )
    )
  );
