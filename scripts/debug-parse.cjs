// Debug parser for Yodel invoice to verify tour extraction in Node (CommonJS)
// Mirrors FinanceInvoiceUpload.tsx parsing logic for daily rows.
const fs = require("fs");
...
const parseDate = (dateStr) => {
  ...
  if (textMatch) {
    ...
    const month = (monthNames.indexOf(textMatch[2].toLowerCase()) + 1)
      .toString()
      .padStart(2, "0");
  ...
  for (let i = 0; i < uniqueDays.length; i++) {
    ...
    for (const p of serviceRowPatterns) {
      ...
      while ((sm = p.regex.exec(dayTextBlock)) !== null) {
        ...
        const qtyNum =
          nums.length > 0 ? parseInt(nums[nums.length - 1].replace(/[^\d]/g, ""), 10) || 0 : 0;
...
const run = async () => {
  ...
  daily.forEach((r) =>
    console.log(`${r.workingDay} ${r.tour} ${r.operatorId} ${r.serviceGroup} qty=${r.qty}`)
  );
};
